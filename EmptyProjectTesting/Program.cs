...
/*
Authentication Scheme kya hoti hai?

ASP.NET Core ek application me multiple login methods support karta hai.
...
Har authentication method ka ek scheme name hota hai.
...
app.Use(next =>
{
    return async context =>
    {
        ...
        if (context.Request.HttpContext.User.FindFirst(ClaimTypes.Role)?.Value == "Admin")
        {

            app.Logger.LogInformation("Admin HiT Request At" + DateTime.UtcNow);
        }
        await next(context);
...
/*Token / Cookie read karta hai.
Token verify karta hai.
Agar valid hai to HttpContext.User fill karta hai.*/
